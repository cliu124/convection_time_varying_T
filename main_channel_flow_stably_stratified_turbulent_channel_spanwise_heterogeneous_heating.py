...
problem.add_equation("dt(u) - 1/Re_tau*div(grad_u) + grad(p) + lift(tau_u2)-Ri_tau*T*ez=-dPdx*ex -dot(u,grad(u))")
problem.add_equation("dt(T) - 1/(Re_tau*Pr)*div(grad_T) + lift(tau_T2)+ 0.5*u@ez = - u@grad(T)")
problem.add_equation("T(y=+1)=0")
problem.add_equation("T(y=-1)=0")
...
snapshots = solver.evaluator.add_file_handler('snapshots_channel', sim_dt=2, max_writes=400, mode=file_handler_mode)
